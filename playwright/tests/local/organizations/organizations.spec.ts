...
test.describe("Organization show", () => {
  ...
  test("View an organization with one practitioner relationship", async ({page}) => {
    ...
    expect(practitionerLink.getAttribute("href")).toBe("/practitioners/f1579a55-b5e1-4717-988d-6e014acbe348")
  })
  test("View an organization with multiple practitioner relationships", async ({page}) => {
    ...
    expect(practitionerLink1.getAttribute("href")).toBe("/practitioners/9b309f46-115e-4eed-bc6e-0e414d5f1215")
    const practitionerLink2 = await page.getByRole("link", {name: "Test Practitioner 2"})
    expect(practitionerLink2.getAttribute("href")).toBe("/practitioners/91cc98f8-8f65-4f6c-8ef2-9dbe829ed5c2")
  })
