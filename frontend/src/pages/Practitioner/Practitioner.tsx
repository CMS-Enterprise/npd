                      ...
                      <TableBody>
                        {Object.entries(obj.locations).map(([id, location]) => (
                          <TableRow key={id}>
                            ...
                            {obj.roleDetails?.telecom?.length !== undefined &&
                            obj.roleDetails.telecom?.length > 0 ? (
                              <TableCell>
                                <strong>
                                  {t("practitioners.detail.locations.phone")}
                                  :{" "}
                                </strong>
                                {
                                  obj.roleDetails?.telecom.filter(
                                    (contact) => contact.system == "phone",
                                  )[0]?.value
                                }
                                <br></br>
                                <strong>
                                  {t("practitioners.detail.locations.fax")}
                                  :{" "}
                                </strong>
                                {
                                  obj.roleDetails?.telecom.filter(
                                    (contact) => contact.system == "fax",
                                  )[0]?.value
                                }
                              </TableCell>
                            ) : (
                              <TableCell>
                                {t("practitioners.detail.locations.noContact")}
                              </TableCell>
                            )}
                          </TableRow>
                        ))}
